...
they are — for a soul a `/dispatch` started, drawn beside the `Agent`-tool agents already there. The
...
seeing it. `useChatRealtimeHandlers` carries `case 'soul_launch_state': return;` beside
`runner_state`, and it must RETURN rather than break: without the case the frame falls through the
switch's `default`, inherits the viewed session's id, and is appended to the open transcript as a
message row.

## The mechanism it shares with the plan runner

The poll itself is not this lane's. `server/shared/polled-lane.service.ts` (`createPolledLane`) is
the read-picture / compare / broadcast-on-change loop that BOTH state lanes on this server run:
this one and the plan runner's (`plan-runner/runner-watcher.service.ts`, now a thin adapter that
supplies its own `snapshot` and `frame` and nothing else). Why it polls rather than watches, when it
speaks, why the dedup records a picture as sent only AFTER the send returns, and why a failing tick
never takes the interval down with it are documented once, there.
...
| `src/modules/chat/transcript/SoulLaunchPinRow.tsx` | One soul's row, drawn to be indistinguishable in SHAPE from the agent rows beside it. What marks it apart is the MARK: `LLMProviderLogo` on the launch's provider — the DeepSeek whale, or Claude's — where an `Agent` subagent carries the robot. In the gutter's Subagents widget the same row is also a button: an `onOpen` prop, given only there and never by the strip, opens this soul's transcript live through the second route above ([architecture/06-tool-view.md](architecture/06-tool-view.md) §Subagents). |
