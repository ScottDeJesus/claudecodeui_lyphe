...
- **檔案瀏覽器** — 互動式檔案樹，文字檔案可用內建編輯器修改，並支援 PDF、Word、試算表、CSV/TSV、Markdown、圖片、音訊和影片的預覽，其餘檔案提供下載 ([file-manager.md](file-manager.md))
- **Git 瀏覽器** — 檢視目前分支、尚未推送的提交，以及每個待處理變更及其 diff；提交與推送交給 Agents：一個按鈕即可開啟對話並執行你自己的 `/git`
...
- **[CodeMirror](https://codemirror.net/)** — PRD 編輯器與檔案分頁檔案編輯器背後的程式碼 / Markdown 編輯器
- **[PDF.js](https://mozilla.github.io/pdf.js/)** — 檔案分頁的 PDF 預覽
- **[docx-preview](https://github.com/VolodymyrBaydalka/docxjs)** — 檔案分頁的 Word 文件預覽
- **[SheetJS](https://sheetjs.com/)** — 檔案分頁的試算表與 CSV/TSV 預覽
