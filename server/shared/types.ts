 ...
 * `ended_at`, `null` until the run ends; `outcome` is the runner's own word (`complete`, `all-blocked`, `budget`, `flag-off`, `dry-run`, `rate-limited`, `unreadable`) and is shown, never branched on beyond its tone. `pid` is the runner process the run
 ...
 * the run ends. It is the only record of a phase the walk left standing on a crash or on the run's budget: that phase's row never turns `blocked`.
 */
...
export type DispatcherPlan = { name: string; v3: string; state: string; status: DispatcherPlanStatus; repo: string; goal: string | null; delivers: string | null; session: string | null; /** The ONE field the server ADDS, which is why this type is not a pure mirror: the plan's `session` resolved through `sessionsDb.resolveAppSessionId` (the rule the run lane's `resolveLaunchingSessions` follows), `null` when the plan names no session or the id resolves to none. The document's own `session` travels untouched beside it. */ session_app_id: string | null; author: string | null; /** The name of the ARC this plan belongs to (`plans.arc_id` read through the arc row), or `null` for a plan of no arc — the join a card makes against the same document's `arcs` list. The bare name, with no `.arc` adornment. */ arc: string | null; /** The plan's EFFECTIVE model word (`dispatcher/model.py:model.of`): its own, else its arc's, else the runner's default — so the control that draws it and presses it back is showing what this plan's next chain will really be launched with, never a bare column. `null` only against a dispatcher build older than the field, which reads the runner's default (`effectiveModelWord`). */ model: RunnerModelChoice | null; created_at: string; updated_at: string; completed_at: string | null; prompted_at: string | null; paused: boolean; /** Whether a walk has EVER gone out for this plan (`report.launched`, read off the plan's own event rows and never stored — INV-172). It is the ONE fact the status word cannot state on its own: `paused` and `scheduled` both describe a plan that is not moving, and neither says whether it was stopped mid-walk or is still waiting at the gate. So it is also the word on the card's primary button — Resume for the first, Start for the second — and the reason an arc's header draws its Resume and its Resume-at controls off the same distinction. `false` against a dispatcher build older than the field. */ launched: boolean; approved: { at: string; by: string } | null; waits_on: string[]; schedule: { start_at: string; unit: string } | null; cost_usd: number; tokens: number; tokens_in: number; tokens_out: number; rounds: number; phases: DispatcherPhase[]; events: DispatcherEvent[] };
/** An arc's derived word, in ONE precedence (`store_arcs.arc_word`): `empty`, `judged`, `complete`, `live`, else `designing`. `empty` comes FIRST because an arc whose every plan was dropped has no plan row for anything below it to read; `judged` next, because the judgment plan IS a plan of its arc; then `complete`, then `live` while any phase of any of its plans is walking (`phase_chain.busy`, never the status column — INV-186). Never a column: INV-172. */
export type DispatcherArcStatus = 'empty' | 'judged' | 'complete' | 'live' | 'designing';
...
export type DispatcherArc = { name: string; goal: string | null; architecture: string | null; delivers: string | null; model: RunnerModelChoice | null; status: DispatcherArcStatus; /** Whether a walk is OUT on any of the arc's plans (`report_arcs.walking` — the `live` ones). This is what `dispatcher stop <arc>` acts on and the set the header draws Stop for. */ walking: boolean; /** Whether any of the arc's plans is STOPPED — `paused` mid-walk, or `scheduled` after having walked (`report_arcs.stopped`). This is what `dispatcher resume <arc>` and `dispatcher schedule <arc>` both act on, and the set the header draws Resume and Resume at 3:00 AM for. READ OFF `launched` and not off `paused` alone: the moment a stopped plan's hour is armed it reads `scheduled`, and a header drawing no control there would offer the operator no way to cancel the very state he is waiting in. */ stopped: boolean; /** The ONE hour the arc's STOPPED plans are armed for, as the timer's own ISO stamp, or null (`report_arcs.hour`). Over the same set the Cancel reaches, never over every plan of the arc: a member armed at the gate is not one the arc's own Cancel can reach, so its hour is not the arc's to name. One stamp because one press wrote them all; null when none of them is armed, and when they were armed apart — the header then offers the press again rather than naming a time half its plans are not waiting for. */ schedule: string | null; plans: string[]; created_at: string; completed_at: string | null; cost_usd: number; tokens: number; tokens_in: number; tokens_out: number };
...
/** The whole picture, pushed on change over `/ws` by the dispatcher lane. It is the document's own keys — `plans`, `arcs`, `route`, `daemon`, `offpeak_at`, `home`, `generated_at` — with the frame's clock added as `at`, epoch MILLISECONDS (`Date.now()`) as `RunnerStateEvent.at`, unlike every time inside. `offpeak_at` is always a stamp — the next DeepSeek off-peak moment, and the literal `none` when the clock cannot answer — so it is a string and never null. */
export type DispatcherStateEvent = { kind: 'dispatcher_state'; arcs: DispatcherArc[]; plans: DispatcherPlan[]; route: DispatcherRoute; daemon: DispatcherDaemon; offpeak_at: string; home: string; generated_at: string; at: number };
/** The verbs this server may relay: `stop`, `resume`, `schedule` (a queued plan's Start at a time — `offpeak`, an ISO timestamp, or `none` to clear), `park` (a designed plan set aside), `unpark` (handed back to be cut) and `model` (a plan's or an arc's own DeepSeek / Claude word; restarts nothing and wakes nobody — the word is read when a chain is launched). */
export type DispatcherVerb = 'stop' | 'resume' | 'schedule' | 'park' | 'unpark' | 'model';
/** What one relayed verb did. A refusal is a RESULT, not an error: the dispatcher prints its refusals on STDOUT (`REFUSED <verb> <name>.v3: <reason>`, exit 2; a not-found line, exit 1 — `no plan <bare>`, or `no plan or arc <bare>` from one of the four verbs an arc's own name also reaches), so `stdout` carries the dispatcher's own first line whole and the reader never gets our paraphrase. `reason` is present only when the dispatcher never got to answer: it timed out, or its binary could not be spawned. */
 ...
 *
 * AND ONE HALF OR THE OTHER, NEVER BOTH (operator rule, 2026-09-24): `cost_usd` is PAID dollars — a soul
 * on the operator's Claude subscription reads `0` there — and `tokens*` are THE SUBSCRIPTION'S, `null`
