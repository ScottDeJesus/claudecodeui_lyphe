...
export type ArcCardSnapshot = { position: number; plan_path: string; title: string; charter: string; run_id: string | null; state: ArcCardState; run_status: string | null; ended_at: number | null; cost_usd: number; spawns: number; phases: ArcCardPhase[]; /** The refusal standing on this card, `null` when none does — the runner's own stamp, copied here unread. A card whose `state` is `stuck` always carries one; a card that was refused and since started carries `null`. */ refusal: ArcCardRefusal | null };
