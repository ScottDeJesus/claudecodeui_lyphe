...
/** One arc of the store (`report_arcs.arc_dict`): its words, its own model word, its derived status and the NAMES of its plans, in `store.arc_plans` order — the arc file's order, except for a member that pre-dated its arc — which is what a reader joins against `plans[].arc` in the same document. `model` is the ARC's own word, never one plan's effective one: it is what the arc's control draws and what `dispatcher model <arc> <word>` hands to every plan of the arc. `cost_usd` and the three token keys are the sums of its plans', read through the same renderers the plan lines use, so an arc and its plans never disagree about what the arc has cost (INV-4299). */
export type DispatcherArc = { name: string; goal: string | null; architecture: string | null; delivers: string | null; model: RunnerModelChoice | null; status: DispatcherArcStatus; plans: string[]; created_at: string; completed_at: string | null; cost_usd: number; tokens: number; tokens_in: number; tokens_out: number };
