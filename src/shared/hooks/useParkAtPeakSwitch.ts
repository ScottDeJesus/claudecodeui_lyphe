...
type ParkAtPeakResponse = { enabled?: boolean };

type ParkAtPeakSwitch = {
...
async function readSwitch(): Promise<boolean | null> {
  try {
    const response = await api.settings.parkAtPeakSwitch();
    const body = (await response.json()) as ParkAtPeakResponse;
    ...
    // `Boolean(undefined)` is `false`, a switch drawn OFF for the very setting that decides whether
    // an Accept walks now, with nothing thrown so the unknown state is never reached. A body without
    // a real boolean is therefore "could not ask", exactly what a thrown call returns.
    if (!response.ok || typeof body.enabled !== 'boolean') return null;
    return body.enabled;
  } catch (error) {
    console.error('Error loading the dispatcher park-at-peak switch:', error);
    return null;
  }
}
...
async function writeSwitch(next: boolean): Promise<boolean | null> {
  try {
    const response = await api.settings.saveParkAtPeakSwitch({ enabled: next });
    const body = (await response.json()) as ParkAtPeakResponse;
    ...
    console.error('Error saving the dispatcher park-at-peak switch:', error);
    return null;
  }
}
 ...
 * file, so it is dropped rather than kept. Unread, not off — and for THIS switch the difference
 * matters in both directions: a row drawing OFF for a switch that is on would promise an Accept that
 * walks immediately, and one drawing ON would promise an hour that is not armed.
 */
...
/**
 * The one reader and writer of the dispatcher's park-at-peak switch in `src/`. The settings row
 * composes it, and any later surface showing the setting composes the same hook, so either can flip
 * it, both show the same answer, and the fetch/save/re-read/error logic exists once.
 *
 * The position is NOT a client preference and so is not `readUserPreference`: it is a file on this
 * host that the dispatcher's own process re-reads at both Accept doors, which is why every read here
 * is a request to the server rather than a value the browser already holds.
 */
export function useParkAtPeakSwitch(): ParkAtPeakSwitch {
