...
import { scheduleClock } from '@/modules/dispatcher/dispatcherState';
import { useOffpeak } from '@/modules/dispatcher/hooks/useOffpeak';
import { Button } from '@/shared/ui';

type ScheduleControlProps = {
  /**
   * Whose Start this schedules: one plan's (`dispatcher schedule <plan>`, a one-shot systemd timer that
   * resumes at the hour), or a dispatch ARC's (`dispatcher schedule <arc>`, one timer per stopped plan
   * of it).
   */
  scope: 'plan' | 'dispatch-arc';
  /**
   * WHICH PLAN VERB THE HOUR WILL SEND, and so what the button is called: `start` (the default) for a plan
   * still waiting at the gate or an arc of them, `resume` for one already stopped mid-walk — which is
   * `PlanControls`' rule and its reason, one word per press. It changes the label, the title and nothing
   * else: the dispatcher's `schedule` verb is the same verb either way, and the timer it arms runs `resume`
   * forever (that word IS "start walking again", which is what a plan at the gate needs too). A dispatch
   * ARC's press chooses nothing by it: its label states what the press does for a plan at the gate and a
   * plan stopped mid-walk alike (`LABEL` below).
   */
  verb?: 'start' | 'resume';
  /** The scheduled moment as the last frame read it off disk (`start_at`, epoch SECONDS), or `null`. */
  startAt: number | null;
  /** A verb is in flight for this plan or arc: the buttons refuse a second press until it answers. */
  busy: boolean;
  /** Relays `offpeak` (schedule) or `none` (cancel). */
  onSchedule: (when: 'offpeak' | 'none') => void;
};

const PREFIX = { plan: 'data-dispatcher-schedule', 'dispatch-arc': 'data-dispatcher-arc-schedule' } as const;
...
// written only where a Resume-at press is a distinct thing — a plan stopped mid-walk. A dispatch ARC's hour
...
// The WORD on the press, by scope. A plan's states WHEN the hour is (`Start at 3:00 AM`, or `Resume at …`
// by the verb that hour will send) — the hour is the off-peak moment and the press arms it. A dispatch
// ARC's says what the press DOES instead: the hour it arms is ONE hour over every plan of the arc that is
// not moving, which is a plan waiting at the gate and a plan stopped mid-walk alike, so one word covers both
...
/**
 * `Start at 3:00 AM` — or `Resume at 3:00 AM`, or, once scheduled, `Cancel`. Used by `PlanControls` (the plan
 * card) and by both decks' own row (`DispatchArcControls`, for a dispatch arc): the same control on both, so
 * one shape means one thing.
 *
 * THE TIME IS THE DISPATCHER'S: `useOffpeak` relays `dispatcher offpeak` (the end of DeepSeek's last daily peak
 * window, derived from `deepseek.PEAK_UTC`), and this control only renders it in the reader's clock. Until the
 * dispatcher answers, the button waits rather than guessing. A plan and a dispatch arc read the same clock, so
 * there is one hour. NOTHING OPTIMISTIC: whether it reads `Start at …` or `Cancel` is `startAt`, the record as
