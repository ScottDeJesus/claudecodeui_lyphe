import { isBody, isFile, isRing } from '@/modules/universe/utils/universeGraph';
...
export function relax(graph: UniverseGraph, A: number): void {
  ...
  // A link is a spring at its rest length — the tree, an import, a co-change, an endpoint. The
  // stiffness is the export's table, already softened by `boundStiffness` for any hub too stiff for
  // this integration to hold.
  ...
  // Pull-home: a ring body is drawn to its ring, a star to the centre of its galaxy. The sun and a
  // node under the hand are pinned — vx and vy are cleared, so nothing accumulates on them. The peak
  // step the loop takes is the graph's speed, which the next frame's temperature reads.
  let speed = 0;
  for (const n of nodes) {
    ...
    if (isRing(n) && isBody(n)) {
      n.vx += (n.tx - n.px) * RING_PULL * A;
      n.vy += (n.ty - n.py) * RING_PULL * A;
    } else {
      n.vx -= n.px * HOME_PULL * A * graph.gv;
      n.vy -= n.py * HOME_PULL * A * graph.gv;
    }
    n.vx *= DAMPING;
    n.vy *= DAMPING;
    n.px += n.vx;
